...
```{r load-packages}
pacman::p_load(ggplot2, readxl, ggbeeswarm, dplyr, tidyverse, cowplot, knitr, install = FALSE)
```
...
# Create the box plot
ggplot(data = cleaned_data, aes(x = factor(number), y = CFU_per_10_leafdiscs, color = number, fill = number)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.3) +
  geom_beeswarm(stroke = 0.5, size = 0.8, alpha = 0.8) +
  labs(title = "CFU per 10 Leaf Discs Across Samples",
       x = "-80 Strain #",
       y = "CFU per 10 Leaf Discs",
       caption = "Data collected during Summer 2024") +
  theme_minimal() +
