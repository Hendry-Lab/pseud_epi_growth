...
```{r load-packages}
pacman::p_load(ggplot2, readxl, BiocManager, RColorBrewer, paletteer, ggbeeswarm, dplyr, tidyverse, devtools, emmeans, cowplot, knitr, survival, here, tibble, survminer, lubridate, formatR, gridExtra, ggsurvfit, ggtree, treeio, gtsummary, tidycmprsk, install = FALSE)
```
...
tree <- read.tree("tree/EpiVir_IQTree_2024july9/EpiVir_DNA_consensus.contree")

# reroot outgroup
# Identify the outgroup tip and its parent node
outgroup_label <- "Outgroup_Ecoli_NR_024570.1"
outgroup_tip <- which(tree$tip.label == outgroup_label)
outgroup_parent_node <- tree$edge[tree$edge[, 2] == outgroup_tip, 1]

# Reroot the tree
rerooted_tree <- root(tree, node = outgroup_parent_node)

# Visualize the tree with customizations
ggtree(rerooted_tree, branch.length="none") + 
